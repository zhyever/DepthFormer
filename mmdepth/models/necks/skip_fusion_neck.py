...
class DepthFusionMultiLevelNeck(nn.Module):
    ...
    def __init__(self,
        ...
        self.level_embed = nn.Parameter(torch.Tensor(4, self.embedding_dim))
        self.query_pos_embed = PositionEmbeddingSine(self.embedding_dim//2)
        self.reference_points = nn.Linear(self.embedding_dim, 2)

        self.multi_att = MSDeformAttn(self.embedding_dim, n_levels=4, n_heads=8, n_points=4)
    ...
    def forward(self, inputs):
        ...
        spatial_shapes = []
        for i in range(len(transformer_feature)):
            bs, c, h, w = transformer_feature[i].shape
            spatial_shape = (h, w)
            spatial_shapes.append(spatial_shape)

            mask = torch.zeros_like(transformer_feature[i][:, 0, :, :]).type(torch.bool)
            masks.append(mask)
            pos = self.feat_pos_embed(transformer_feature[i], mask)
            mask = mask.flatten(1)
            pos_embed = pos.flatten(2).transpose(1, 2)
            lvl_pos_embed = pos_embed + self.level_embed[i].view(1, 1, -1)

            feat = self.proj_conv[i](transformer_feature[i])
        ...
        tgt_with_embed = tgt + query_embed
        reference_points_input = reference_points[:, :, None] * valid_ratios[:, None]
        
        fusion_res = self.multi_att(tgt_with_embed, reference_points_input, src_flatten, spatial_shapes, level_start_index)
        fusion_res = fusion_res.permute(0, 2, 1)
        fusion_res = fusion_res.reshape(bs, c, w, h)
        outs.insert(0, self.conv_fusion(fusion_res))
